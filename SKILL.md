...
- **表格结构与源文逐行对账**：每一行的单元格数必须等于分隔行列数。合并表头在解析时
  会丢掉重复列（如 `Work surface` 应跨 4 列却只剩 3 格），必须按子表头（第二行）的
  列数反推分组跨度并用显式覆盖修正，禁止在渲染器里猜重复列。
- **单表列数上限 10 列**（乐享硬限制，超限会在写入中途被拒并留下半成品页面）：
  超限表按源文自身的能力分组拆成多张子表，每张保留第 0 列作行标签列，并在题注后补
  一条双语注记说明原表列数与拆分依据（列序/行序/取值与原文一致）。
- **行内公式就地物化为 Unicode**：`\mathcal{T}`→`𝒯`、`\in`→`∈`、`\sum`→`Σ`、
  `\bar{S}`→`S̄`、`_{m,i}`→`ₘ,ᵢ`、`^{-1}`→`⁻¹`；转义花括号 `\{...\}` 是集合语义，
  必须先占位保护再还原。物化后断言不残留 `\`。
- 无法物化的独立展示公式预渲染为 PNG，用
  `![公式(N)/Equation(N)](images/eqNN.png) **公式（N）/ Equation（N）**：中文释义` 承载；
  上传前终稿不应再有 `$`（预检 JSON 的 `formulas` 为 0）。
...
- 表格逐行结构与源文一致（列数与分隔行相同），合并表头的重复列已按子表头补齐。
- 每张表的列数 ≤ 10；超限表已按能力分组拆分并附双语注记。
- 全文无残留 `$`（行内公式已物化为 Unicode，展示公式已转 PNG）；图片数本地与线上一致。
- 所有源文本地图片路径不变、顺序不变、文件存在；且不存在 `![](x1,y1,x2,y2)` 形式的
  坐标伪引用（矢量图漏解析的信号）。
